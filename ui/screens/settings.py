...
class SettingsScreen(Page):
    def __init__(self, master, storage, focus_on, on_focus, on_reset, on_theme, appearance="Light",
        ...
        ai_card = self._card(
            "עזרה מקומית",
            "מנסחת שאלות בשפה פשוטה ומסבירה שלב־שלב בתוך התרגול. רצה אצלכם, בלי מפתח API. האנליסט בוחר מה לתרגל.",
        )
        self._toggle(
            ai_card,
            "עזרה מקומית: דולקת" if ollama_on else "עזרה מקומית: כבויה",
            ollama_on,
            on_ollama,
        )
        self._toggle(
        ...
        if ollama_on:
            tip = (
                "כשדולקת: בתרגול יש «בשפה פשוטה» ו«שלב־שלב». "
                "אם אין תשובה תוך כ־40 שניות — עוברים למצב מקומי. "
                "צריך Ollama רץ ואת המודל (למשל qwen2.5:3b)."
            )
        else:
            tip = "כבויה: תרגול רגיל. אפשר להדליק בכל רגע. האנליסט ממשיך להתאים שאלות גם בלי זה."
